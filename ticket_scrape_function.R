...
get_seatgeek_data <- function() {
  ...
  for(mlb_pages in 1:10) {
    ...
    home_team <- mlb_dat |> 
      filter(!is.na(performersnewhome_team)) |> 
      select(id, home_team = performersnewslug)
    
    away_team <- mlb_dat |> 
      filter(!is.na(performersnewaway_team)) |>
      select(id, away_team = performersnewslug)
    
    game_data <- full_join(home_team, away_team, by = "id")
